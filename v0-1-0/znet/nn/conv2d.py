...
class Conv2d(Module):
    def __init__(self, in_channels, out_channels, kernel_size,
                 stride=1, padding=0, dilation=1, groups=1, bias=True, dtype=mx.float32):
        super().__init__()
        ...
        w = mx.random.uniform(
            shape=(out_channels, in_channels // G, kH, kW),
            low=-bound, high=bound, dtype=dtype
        )
        self.add_parameter("weight", Tensor(w, requires_grad=True, dtype=dtype))

        if bias:
            self.add_parameter("bias", Tensor(mx.zeros((out_channels,), dtype=dtype), requires_grad=True))
        else:
