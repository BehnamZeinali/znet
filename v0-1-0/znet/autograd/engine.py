...
class Context:
    ...
    def save_for_backward(self, *arrs):
        self.saved_tensors = tuple(np.array(a) for a in arrs)
...
def engine_backward(output_tensor, grad, *, retain_graph=False):
    ...
    grads = {output_tensor: np.asarray(grad, dtype=output_tensor.data.dtype)}
    ...
    for t in reversed(topo):
        ...
        for parent, gin in zip(node.parents, in_grads):
            if getattr(parent, "requires_grad", False) and gin is not None:
                _accumulate(grads, parent, np.asarray(gin, dtype=parent.data.dtype))
