...
def _promote_for_matmul(a, b):
    ...
    a_exp = a[np.newaxis, :] if a_was_1d else a
    b_exp = b[:, np.newaxis] if b_was_1d else b
...
class Matmul(Function):
    ...
    def forward(ctx, a, b):
        ...
        y_exp = a_exp @ b_exp  # NumPy handles leading-dim broadcasting
    ...
    def backward(self, grad_out):
        ...
        a_exp = a[np.newaxis, :] if a1d else a
        b_exp = b[:, np.newaxis] if b1d else b

        # Unsqueeze grad to pre-squeeze shape
        g = np.reshape(grad_out, y_exp_shape)

        # Raw grads in broadcasted shapes
        ga = g @ np.swapaxes(b_exp, -1, -2)
        gb = np.swapaxes(a_exp, -1, -2) @ g

        # Remove singleton axes introduced by 1D promotion
        if a1d and ga.ndim > a.ndim:
            ga = np.squeeze(ga, axis=-2)  # (1,D) -> (D,)
        if b1d and gb.ndim > b.ndim:
            gb = np.squeeze(gb, axis=-1)  # (D,1) -> (D,)
