...
def train(model, criterion, optimizer, epoch):
    ...
    for i in range(iters_per_epoch):
        ...
        data_batch = Tensor(train_data[batch_idx], requires_grad=False, device=device)
        target_batch = Tensor(train_labels[batch_idx].astype(np.int64), requires_grad=False, device=device)
...
def evaluate(model, test_data, test_labels):
    ...
    # no-grad context (if your API provides one)
    try:
        from znet._grad_mode import no_grad
        grad_ctx = no_grad()
    except Exception:
    ...
    with grad_ctx:
        for i in range(num_batches):
            data  = Tensor(test_data[i * batch_size : (i + 1) * batch_size], requires_grad=False, device=device)
            target = Tensor(test_labels[i * batch_size : (i + 1) * batch_size].astype(np.int64),
